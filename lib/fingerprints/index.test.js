...
describe('Hash fingerprints plugin', () => {
  ...
  it('does not sync fingerprints for source map metadata', () => {
    expect(metadata.fingerprints['example-source-map.css'].hash)
      .not.toEqual(metadata.fingerprints['example-source-map.css.map'].hash)

    expect(metadata.fingerprints['example-source-map.js'].hash)
      .not.toEqual(metadata.fingerprints['example-source-map.js.map'].hash)
  })
