  ...
  return function (files, metalsmith, done) {
    ...
    for (const item in navigation) {
      ...
      if (navigation[item].url && navigation[item].url !== '') {
        // define source path
        const itemPath = navigation[item].url
        // get directories under main navigation item (e.g. ['breadcrumbs', 'checkboxes', ...])
        const directories = metalsmith.match(`${itemPath}/*/*.*`, Object.keys(files)).map(p => {
          const subdirStart = itemPath.length + '/'.length
          return dirname(p).slice(subdirStart)
        })
        ...
        if (directories) {
          ...
          for (const dir in directories) {
            const url = join(navigation[item].url, directories[dir])
            const frontmatter = files[join(url, 'index.html')]
