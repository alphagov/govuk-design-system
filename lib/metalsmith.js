...
const markdown = require('./metalsmith-remark')              // convert markdown files to html
...
module.exports = metalsmith(__dirname) // __dirname defined by node.js: name of current working
  ...
  .use(markdown({
    sanitizeSchema: {
      // Don't clobber generated IDs as some markup relies on them.
      clobberPrefix: '',
      tagNames: [
        'span',
        'iframe'
    ...
    classNames: {
      ...
      list (node) {
        ...
        classNames.push(node.ordered ? 'govuk-list--number' : 'govuk-list--bullet')
        classNames.push('app-!-max-width-measure')
        return classNames
      },
      paragraph: [
      ...
      image: ['app-img app-!-max-width-measure'],
