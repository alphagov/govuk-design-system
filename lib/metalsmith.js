...
const fileTracker = require('../lib/custom-watcher')
const nunjucksOptions = require('../lib/nunjucks/index.js') // nunjucks options

// Local metalsmith plugins
const runOnlyOn = require('./custom-watcher/run-only-on')
const runOnlyOnTransformed = require('./custom-watcher/run-only-on-transformed')
const skipUnless = require('./custom-watcher/skip-unless')
const trackModifications = require('./custom-watcher/track-modifications')
...
const tracker = trackModifications()
...
module.exports = metalsmith
  ...
  .use((files, metalsmith, done) => {
    fileTracker.updateMetadata(files, metalsmith)
    done()
  })

  .use(tracker.recordChanges) // Track which files we'll modify
  ...
  .use(
    runOnlyOnTransformed('**/*.html', layouts, {
      default: 'layout.njk',
