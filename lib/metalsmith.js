...
module.exports.merge = function () {
  ...
  const stagingJS = resolve(__dirname, '../.staging/javascripts')
  ...
  // Copy Assets
  if (existsSync(stagingJS)) {
    cpSync(stagingJS, buildDir, { recursive: true })
  }
}
...
module.exports.compileJavaScript = async function () {
  await cache
    .metalsmith(tracer(Metalsmith(resolve(__dirname, '../'))))
    .source(paths.source)
    .destination(join(paths.staging, 'javascripts'))
    .env(env)
    .metadata(metadata)
    .ignore([
      '**/*.md',
    ...
    .use((files, metalsmith, done) => {
      ...
      return hashAssets({
        pattern: ['javascripts/*.{cjs,js,mjs}?(.map)', 'javascripts/vendor/*']
      })(files, metalsmith, done)
    })
    .build()
}
