...
// extract page headings
  ...
  .use((files, metalsmith, done) => {
    const copyAssets = (pattern, options) => glob.sync(pattern, options).forEach((asset) => {
    ...
    copyAssets('{fonts,images}/*', {
      cwd: path.join(paths.nodemodules, '@hmlr/frontend/hmlr/assets'),
      dest: 'assets'
    })
  ...
  .use(canonical({
    hostname: 'https://hmlr-design-system.herokuapp.com/',
    omitIndex: true,
