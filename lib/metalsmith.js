...
module.exports = metalsmith
  ...
  .use(async (files) => {
    const sectionRoots = Object.values(files)
      // RESTART HERE: Could we use another property so we could
      // order the files using the same `order` property
      // as done in the subnav
      .filter((file) => !!file.sectionRoot)
      .toSorted((a, b) => a.sectionRoot - b.sectionRoot)
      // RESTART HERE: Don't map the `title` and `permalink`
      // instead pass the whole file to the navigation plugin
      .map(({ title, permalink }) => ({
        label: title,
        url: permalink
      }))

    // Clear the existing items for safety when running in dev mode
    menuItems.splice(0, menuItems.length)
    menuItems.push(...sectionRoots)
  })
