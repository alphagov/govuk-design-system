...
const transformer = require('./branding-transformer.js') // used to build GOV.UK Frontend JavaScript
...
module.exports = metalsmith
  ...
  .use(async (files, metalsmith, done) => {
    Object.keys(files).forEach((file) => {
      if (files[file].rebrand) {
        ...
        files[newFile].layout = `branded/${files[file].layout}`
      }
    })
    done()
  })
  ...
  // render branded templates with a custom transformer
  .use(
    inPlace({
      pattern: '**/*branded.njk',
