...
const { default: linter } = require('metalsmith-html-linter') // debug logging
...
module.exports = metalsmith
  ...
  .use(
    linter({
      html: ['**/*.html', '!**/google*.html'],
      linthtml: {
        rules: {
          // https://github.com/linthtml/linthtml/blob/develop/doc/docs/user-guide/rules/list.md
          'id-class-no-ad': false, // Checks for IDs that might get flagged by ad blockers
          'img-req-alt': false, // Requires non-empty img alt. Struggles with presentational images
          'label-req-for': false // Struggles with the search input, which is created via JS
        }
      }
    })
  )
