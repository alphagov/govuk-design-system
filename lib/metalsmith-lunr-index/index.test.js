...
describe('metalsmith-lunr-index plugin', () => {
  ...
  beforeAll((done) => {
    Metalsmith('lib/metalsmith-lunr-index/fixtures')
      ...
      .use(
        plugin({
          items: [{ label: 'Components' }, { label: 'Patterns' }]
        })
      )
      .build((err, files) => {
        if (err) {
  ...
  describe('the generated document store', () => {
    ...
    it('contains the permalink to the document', () => {
      const entry = Object.values(documentStore).find(
    ...
    it('contains the page and heading level 2 entry', () => {
      const entry = Object.values(documentStore).find(
        ({ title }) => title === 'Heading level 2'
      )

      expect(entry.permalink).toEqual(
        'with-page-headings.html/#heading-level-2'
      )
    })

    it('does not contain headings from documents in excluded sections', () => {
      const paths = Object.keys(documentStore).filter((path) =>
        path.startsWith('about-larry.html')
      )
  ...
  describe('the generated index', () => {
    ...
    it('stores the aliases of the page in the metadata', () => {
      ...
      expect(documentStore[resultRef].aliases).toEqual(
