...
module.exports = function lunrPlugin () {
  return (files, metalsmith, done) => {
      ...
      Object.keys(files)
        ...
        .map(path => {
          ...
          return {
            ...
            backlog: file.backlog === null ? undefined : file.backlog
          }
        })
    ...
    const pageHeadingDocuments =
      ...
      .map(path => {
        let file = files[path]
        let documentPath = file.path || path
    ...
    const index = lunr(function () {
      ...
      pageHeadingDocuments.forEach(doc => {
        let separator = '<span class="app-site-search__separator" aria-hidden="true">›</span>'
        store[doc.path] = {
          title: doc.title,
          path: doc.path,
