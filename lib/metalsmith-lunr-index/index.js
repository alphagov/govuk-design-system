...
const navigationConfig = require('../../config/navigation')
    ...
    const index = lunr(function () {
      ...
      documentResults.forEach((doc) => {
        store[doc.permalink] = {
          ...
          title: doc.title,
          section: `in <strong>${doc.section}</strong>`,
          aliases: doc.aliases
        }
        this.add(doc)
      })
      ...
      pageHeadingDocuments.forEach((doc) => {
        store[doc.permalink] = {
          permalink: doc.permalink,
          title: doc.title,
          section: `<span class="govuk-visually-hidden">,</span> part of <strong>${doc.page}</strong> in <strong>${doc.section}</strong>`,
          aliases: doc.aliases
        }
        this.add(doc)
      })
