...
describe('FileTracker', () => {
  ...
  describe('runOnMatchedFiles', () => {
    let mockPluginFactory

    beforeEach(() => {
      mockPluginFactory = jest.fn((options) => {
        return jest.fn((files, metalsmith, done) => done())
      })
    })

    it('runs plugin with original options when changedFiles is null', (done) => {
      tracker(files, metalsmith)

      const options = { pattern: ['**/*.txt'], foo: 'bar' }
      const check = fileTracker.runOnMatchedFiles(
        ['**/*.txt'],
        mockPluginFactory,
        options
      )
      ...
      check(files, metalsmith, () => {
        expect(mockPluginFactory).toHaveBeenCalledWith(options)
        done()
      })
    })

    it('runs plugin with matched files pattern when files match', (done) => {
      files['page.njk'] = { contents: Buffer.from('{% block %}') }
      tracker(files, metalsmith)

      const options = { pattern: ['**/*.{md,njk}'], transform: 'nunjucks' }
      const check = fileTracker.runOnMatchedFiles(
        ['**/*.{md,njk}'],
        mockPluginFactory,
        options
    ...
    it('skips plugin when no files match pattern', (done) => {
      files['styles.css'] = { contents: Buffer.from('body { }') }
      tracker(files, metalsmith)
      ...
      check(files, metalsmith, () => {
        expect(mockPluginFactory).not.toHaveBeenCalled()
        done()
      })
    })

    it('handles multiple matched files', (done) => {
      files['page1.md'] = { contents: Buffer.from('# Page 1') }
      files['page2.md'] = { contents: Buffer.from('# Page 2') }
      files['style.css'] = { contents: Buffer.from('body { }') }
      tracker(files, metalsmith)

      const options = { pattern: ['**/*.md'] }
      const check = fileTracker.runOnMatchedFiles(
        ['**/*.md'],
        mockPluginFactory,
        options
      ...
      check(files, metalsmith, () => {
        expect(mockPluginFactory).toHaveBeenCalledWith({
          pattern: expect.arrayContaining(['page1.md', 'page2.md'])
        })
        expect(mockPluginFactory.mock.calls[0][0].pattern).toHaveLength(2)
        done()
      })
    })

    it('preserves other options when modifying pattern', (done) => {
      files['component.njk'] = { contents: Buffer.from('{{ content }}') }
      tracker(files, metalsmith)

      const options = {
        pattern: ['**/*.njk'],
        transform: 'nunjucks',
        engineOptions: { autoescape: true }
      }
      const check = fileTracker.runOnMatchedFiles(
        ['**/*.njk'],
        mockPluginFactory,
    ...
    it('handles glob patterns with multiple extensions', (done) => {
      files['page.md'] = { contents: Buffer.from('# Page') }
      files['template.njk'] = { contents: Buffer.from('{{ content }}') }
      tracker(files, metalsmith)
      ...
      check(files, metalsmith, () => {
        expect(mockPluginFactory).toHaveBeenCalledWith({
          pattern: expect.arrayContaining(['page.md', 'template.njk'])
        })
        done()
      })
    })

    it('works with empty options object', (done) => {
      files['file.txt'] = { contents: Buffer.from('content') }
      tracker(files, metalsmith)
