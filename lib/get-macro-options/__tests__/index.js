...
const fixtures = {
  [join(packagePath, 'components/radios/macro-options.json')]: [
    ...
    {
      description:
        'Options for a component that does not exist in the Design System (hint component).',
      isComponent: true,
      name: 'hint',
      required: false,
      type: 'object'
    ...
    {
      description:
        'Options for a component that does not exist in the Design System (label component).',
      isComponent: true,
      name: 'label',
      required: false,
      type: 'object'
    ...
    },
    {
      ...
      params: [
        {
          description:
            'Optional classes to add to the form group (e.g. to show error state forthe whole group)',
          name: 'classes',
          required: false,
          type: 'string'
        }
      ...
      params: [
        {
          ...
          params: [
            ...
            {
              description:
                'Options for a nested component that does not exist in the Design System (label component).',
              isComponent: true,
              name: 'label',
              required: false,
              type: 'object'
            ...
            {
              description:
                'Options for a nested component that exists in the Design System.',
              isComponent: true,
...
describe('getMacroOptions', () => {
  ...
  describe('slugs', () => {
    ...
    it('appends slugs to nested options', () => {
      expect(output[2].options[0].params[2].slug).toBe(
        'nested-component-with-camel-case-name'
      )
    })
  })
  ...
  describe('additional components', () => {
    ...
    it('should only output additional components once', () => {
      const optionsWithLabelInName = Object.entries(output)
        .map((entry) => {
  ...
  describe('markdown rendering', () => {
    it('renders descriptions as markdown', () => {
      expect(output[2].options[0].description).toBe(
        'items for nested <a href="href">link</a>'
      )
    })
    it('renders nested options descriptions as markdown', () => {
      expect(output[2].options[0].params[0].description).toBe(
        '<a href="href">link</a>'
      )
    })
  })
