...
exports.getMacroArguments = path => {
  ...
  try {
    macroArguments = require(__dirname + '/../' + path).arguments
  } catch (err){}

  let processedArguments = []

  let processArguments = (groupName, rawArguments) => {
    let group = {
      "groupName": groupName,
      "arguments": []
    }

    processedArguments.push(group)

    for (let item of rawArguments){
      item = Object.assign({}, item)
      if (item.arguments){
        item.description += ` See <a href="#nunjucks-macro-arguments-${item.name}">${item.name}</a>.`
        processArguments(item.name, item.arguments)
      }
      group.arguments.push(item)
    }
  }

  processArguments("", macroArguments)

  console.log(JSON.stringify(processedArguments, null, '  '))
