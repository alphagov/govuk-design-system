...
function getDuplicateTitles(groupedFiles) {
  ...
  for (const title in groupedFiles) {
    ...
    const groupedWithoutExcludedFiles = group
...
module.exports = function titleChecker() {
  return (files, metalsmith) => {
    const templateFileNames = Object.keys(files)
      // Filter files that are Nunjucks, which have frontmatter.
      .filter(
        (filename) => filename.endsWith('.njk') || filename.endsWith('.md')
      )

    const groupedFilesByTitle = groupFilesByTitle(templateFileNames, files)
    ...
    if (Object.keys(filesWithDuplicateTitles).length) {
      ...
      for (const title in filesWithDuplicateTitles) {
        ...
        duplicateErrorMessage += group
          .map((file) => `- ${file.__filename}`)
          .join('\n')
    ...
    const filesWithoutTitles = templateFileNames.filter(
      (filename) => !files[filename].title
    )
