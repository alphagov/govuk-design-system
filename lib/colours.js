...
const palette = function () {
  ...
  const paletteMap = !('paletteMap' in palette)
    ? variables.find(({ compiledValue, mapValue }) => compiledValue === palette.compiledValue && mapValue).mapValue
    : palette.mapValue

  // Convert Sass map entries into a single object
  return Object.fromEntries(paletteMap
    .map(({ name, compiledValue }) => [name, compiledValue]))
}
