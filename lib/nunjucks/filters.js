...
/**
 * The opposite of the slugify filter
 *
 * For when you have access to a "slug" style string but need to convert it into
 * a title of a page eg: of a component for use in guidance.
 *
 * @param {string} string - String to format
 * @returns {string} - Title string for use in guidance
 */
exports.titlify = function (string) {
 ...
 * @param {{url: string}} navigationItem - The navigation item
 * @param {string} permalink - The permalink of the page being rendered
 * @returns {{url: string}|undefined|boolean} - the active subpage if the current
 *   page is active, undefined if the navigation item's subpages don't contain
 *   the current page, false if the navigation item doesn't have any subpages
 */
exports.getActiveSubpage = function (navigationItem, permalink) {
 ...
 *
 * @param {string} string - The string of markdown to render
...
/**
 * Format a date to a short prosaic format (e.g. Feb 2026)
 * for display in changelog tables, where space is limited.
 *
 * @param {string} isoDate - The date in ISO 8601 format (YYYY-MM-DD)
 * @returns {string} - The date in MMM YYYY format
 */
