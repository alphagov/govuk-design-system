...
const { getChangelog } = require('../get-changelog/index.js')
...
/**
 * Get the slug of the current page, without any parent sections.
 *
 * Useful for instances where the title of a page is different from the page URL,
 * meaning that the slugify filter won't return what you're looking for
 * eg: `components/footer` and 'GOV.UK Footer'
 *
 * @returns {string} - The slug of the current page
 */
exports.getCurrentPageSlug = function () {
  const permalink = this.lookup('permalink')

  // Match permalink against the last instance of a directory following a slash
  // (thanks to `$`), accounting for trailing slashes.
  // Trailing slashes shouldn't happen in permalinks but including as a precaution.
  const dirname = permalink.match(/\/[a-z-]*\/*$/)

  // Remove any slashes captured in the match, if match was successful.
  return dirname && dirname[0].replaceAll('/', '')
}
